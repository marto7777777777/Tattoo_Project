...
function CreateArtistResponsePage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await createArtistResponse({
        tattooRequestId: Number(form.tattooRequestId),
  ...
  async function handleReject() {
    ...
    try {
      const response = await rejectTattooRequest(
        Number(form.tattooRequestId)
      );
      const data = await readResponse(response);
      ...
      setSuccess("Tattoo request rejected successfully.");
    } catch {
      setError("Server connection failed. Please try again.");
    }
  }
    ...
    <main className="center-container artist-action-page">
      <section className="card form-card artist-action-card">
        <div className="header">
          ...
          <p>Create an acceptance response or reject the request.</p>
        </div>
        ...
        <form className="form" onSubmit={handleSubmit}>
          ...
          <p className="artist-estimate-notice">
            Price and duration are preliminary estimates. The final price and
            required time are confirmed after the consultation.
          </p>
