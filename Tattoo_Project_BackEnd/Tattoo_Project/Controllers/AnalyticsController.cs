...
public class AnalyticsController(TattooDbContext db):ControllerBase
{
 [HttpPost("pending-events/consume")]public async Task<IActionResult> Consume(){var uid=User.FindFirstValue(ClaimTypes.NameIdentifier);var artistId=await db.TattooArtists.Where(x=>x.UserId==uid).Select(x=>(int?)x.Id).FirstOrDefaultAsync();if(artistId==null)return Ok(Array.Empty<AnalyticsEventDto>());var items=await db.AnalyticsOutboxEvents.Where(x=>x.TattooArtistId==artistId&&x.DispatchedAt==null).OrderBy(x=>x.Id).Take(100).ToListAsync();var result=items.Select(x=>new AnalyticsEventDto{Name=x.Name,CompletedProjectCount=x.CompletedProjectCount}).ToList();foreach(var item in items)item.DispatchedAt=DateTime.UtcNow;await db.SaveChangesAsync();return Ok(result);}
}
